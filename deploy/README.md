...
- **ASP.NET Core Runtime 10.0.11 oder neuer in der 10.x-Linie (x64)** — Download unter <https://dotnet.microsoft.com/download>. Die reine Runtime genügt (Kestrel hostet selbst); das **Hosting Bundle nur, wenn bewusst IIS im Spiel ist** — es verdrahtet IIS und startet W3SVC neu, auf geteilten Hosts (SCCM/WSUS) unerwünscht. Das `(x64)` ist keine Empfehlung: NodePilot wird als `win-x64` veröffentlicht, eine 32-Bit- oder ältere verwundbare 10.x-Runtime wird vom Preflight ausdrücklich zurückgewiesen
...
| „Processes are still running from … and could not be ended" | Ein Prozess aus dem Install-Verzeichnis hat 30 s Gnadenfrist überlebt **und** ließ sich nicht beenden — praktisch immer fehlende Rechte oder ein hängender Kernel-Aufruf. PID und Name stehen in der Meldung; Dienst und Dateien sind unangetastet. Prozess beenden oder neu starten, dann erneut. |
...
| 503 auf `/healthz/ready` | Datenbank nicht bereit. Zustand und Grund stehen ohne Login unter `/healthz/database` (immer HTTP 200) oder in `np health`. SQL Server zusätzlich als gMSA mit `sqlcmd -S sql01 -E -d NodePilot -Q "SELECT 1"` prüfen. |
| DB-Ausfall **im laufenden Betrieb** | Erwartet: Der Dienst bleibt oben, `/api` antwortet `503 DATABASE_UNAVAILABLE`, die UI zeigt den Ausfall und NodePilot fährt nach erfolgreichen Probes automatisch fort. Workflows warten an der Durability-Grenze; währenddessen beobachtete Trigger werden gezählt und ohne Catch-up verworfen. Details: [ADR 0011](../docs/adr/0011-database-availability-breaker.md). |
| `/healthz/database`: `RejectedByServer` | Credentials, Datenbank/Katalog oder TLS korrigieren; Warten oder ein Neustart allein behebt diesen Zustand nicht. Nach einer serverseitigen Korrektur greift die Sonde automatisch; geänderte NodePilot-Verbindungsdaten benötigen einen Dienst-Neustart. |
