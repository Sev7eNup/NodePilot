...
internal sealed class ScorchApiClient : IScorchApiClient, IDisposable
{
    ...
    private async Task<IReadOnlyList<T>> GetCollectionAsync<T>(string path, CancellationToken cancellationToken)
    {
        ...
        while (!string.IsNullOrWhiteSpace(next))
        {
            ...
            if (root.ValueKind != JsonValueKind.Array)
                throw new InvalidOperationException($"SCOrch returned an unexpected {typeof(T).Name} response.");
            items.AddRange(JsonSerializer.Deserialize<List<T>>(root.GetRawText(), new JsonSerializerOptions
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        throw new InvalidOperationException(
            $"Could not {operation}: HTTP {(int)response.StatusCode} {response.StatusCode}. {detail}".Trim());
    }
}
