...
/// <i>invoke</i> methods — <c>JoinExecution</c> and <c>JoinWorkflow</c> read the database — against a
/// wedged server. This filter answers immediately instead, with the same stable code the HTTP surface
/// uses, so the client can tell "database outage" from "bug".</para>
///
/// <para>Server-to-client pushes are unaffected — filters only run for client-to-server invocations.</para>
/// </summary>
public sealed class DatabaseAvailabilityHubFilter(IDatabaseAvailability availability) : IHubFilter
{
        ...
        Func<HubInvocationContext, ValueTask<object?>> next)
    {
        ...
        catch (Exception ex)
        {
            ...
            // Connection interceptors open the breaker before the provider exception reaches us. Let
            // that stronger process-wide fact win even when the provider wrapped the original failure
            ...
            // IOException, TimeoutException and pool-shaped InvalidOperationException are not database
            // types. Hub methods may throw any of them for unrelated work, so classification alone is
            // not enough to replace the original exception with a database protocol code. Armed state,
