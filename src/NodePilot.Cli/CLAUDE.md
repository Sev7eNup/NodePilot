...
Reiner HTTP-Client gegen die REST-Endpoints (Spectre.Console.Cli), mit beiden Installern ausgeliefert (`<install>\tools\np`, Maschinen-PATH via `deploy/MachinePath.ps1`), aus dem Checkout per `dotnet publish` — **kein** `dotnet global tool` (`PackAsTool` × `net10.0-windows` = NETSDK1146). Befehlsbereiche: `auth`, `workflow`, `exec`, `machine`, `credential`, `globals`, `user`, `shared-folder`, `maintenance`, `alerting`, `system-alert`, `audit`, `backup`, `db`, `cron`, `health`, `dashboard`, `failure-causes`, `operations`, `observability`, `settings`, `secrets`, `config`. Details: `docs/claude-reference.md`.
