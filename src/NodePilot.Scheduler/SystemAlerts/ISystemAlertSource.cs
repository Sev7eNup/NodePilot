...
/// <see cref="SystemAlertSourceDescriptor"/> and, given a descriptor-validated <see
/// cref="SystemAlertQuery"/>,
/// yields raw <see cref="SystemAlertObservation"/>s WITHOUT deciding severity or health — the
/// central
...
/// SystemAlertSourceStates tables (wired in a later phase). Sources are compiled and DI-registered
/// — never
/// user-uploaded, never a text query language.
/// </summary>
public interface ISystemAlertSource
{
    /// <summary>Stable identity; MUST equal <c>Describe().SourceId</c> (guarded by
    ...
    /// <summary>
    /// Whether this source can produce observations right now (e.g. the underlying feature is
    /// enabled /
    ...
    /// <summary>
    /// Sample current observations for the given descriptor-validated parameter set. Read-only —
    /// must not
    /// mutate the database or any external system (the write side is the evaluator + delivery
    /// pipeline).
    /// </summary>
