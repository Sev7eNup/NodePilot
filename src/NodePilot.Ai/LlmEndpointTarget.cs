...
/// <summary>
/// Which OpenAI wire dialect an endpoint speaks. Deliberately <b>not</b> a config field: the
/// operator already states it by typing the URL, and the two dialects are addressed under distinct
/// paths (<c>/chat/completions</c> vs. <c>/responses</c>). See
...
/// <summary>
/// A configured LLM <c>BaseUrl</c> after validation and dialect detection — the resolved answer to
/// "where do I POST, what does it speak, and where do sibling paths hang off".
/// </summary>
/// <param name="PostUrl">The exact URL completions are POSTed to.</param>
/// <param name="ApiRoot">
/// The endpoint root, i.e. <see cref="PostUrl"/> minus its dialect suffix. Sibling paths hang off
/// this — currently only the settings test-probe's <c>/models</c>.
/// </param>
