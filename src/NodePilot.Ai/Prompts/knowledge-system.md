...
- **Instance database (read-only SQL / text2sql)** — data stored in this NodePilot instance's own database (how many workflows, executions, machines, users …). Use `list_db_tables` to discover the SQL provider/schema, `get_db_table` for columns and foreign-key joins, then `execute_readonly_sql` to run a single read-only statement. Always use the returned provider dialect and real `DbTableName`.
