...
{
    /// <summary>
    /// Returns <c>false</c> to veto a retry outright, <c>true</c> to let the provider's own
    /// strategy
    /// decide as before.
    ///
    /// <para><b>Never retry a command timeout.</b> This is the single highest-value line in the
    /// whole
    ...
    /// first time.</para>
    ///
    /// <para><b>Never retry while the breaker is open.</b> The database is known to be gone; six
    /// more
    /// attempts only hold a connection and a thread-pool continuation for minutes.</para>
    ///
    /// <para>Everything else still delegates to the provider's own transient-error list, so
    /// deadlocks,
    ...
    /// <c>WorkflowDbWriteMetrics.SaveChangesIdempotentAsync</c> that makes those retries
    /// safe.</para>
    ///
    /// <para>While <see cref="DatabaseAvailabilityState.Booting"/> and
    /// <see cref="DatabaseAvailabilityState.Armed"/> the answer is "delegate": the boot block must
    /// keep
    /// its full resilience against a database that is still coming up, and Armed is by definition
    /// the
...
/// <para>Derives from <see cref="SqlServerRetryingExecutionStrategy"/> rather than replacing it so
/// the
...
/// because the two providers' base strategies take different final constructor arguments and C# has
/// no
