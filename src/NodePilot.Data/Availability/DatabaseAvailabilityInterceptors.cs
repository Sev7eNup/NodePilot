...
///
/// <para>Verified on 2026-08-06 against EF Core 10.0.10 + Npgsql 10.0.3: this hook fires <b>once
/// per
/// attempt inside</b> the retrying execution strategy — <c>EnableRetryOnFailure(2)</c> produced
/// three
/// calls, not one. The breaker therefore trips on the first failed attempt rather than after the
/// whole
...
/// <item><c>ConnectionCanceled*</c> — a caller's token firing says nothing about the
/// database.</item>
/// </list></para>
///
    ...
    : DbConnectionInterceptor
{
    ...
    {
        // ClassifyConnectionFailure, not Classify: a connect timeout and a command timeout arrive
        // in
        // the identical exception shape (NpgsqlException wrapping TimeoutException, measured), so
        // the
        ...
        switch (kind)
        {
            // The server is alive and out of connection slots. Opening the breaker here would turn
            // a
            ...
            // installation. Arm the dedicated SELECT-1 probe; its positive round trip adjudicates
            // the
...
///
/// <para>A command timeout deliberately does <b>not</b> open the breaker. It cannot: one slow query
/// is
/// indistinguishable from a wedged server at the exception level, and the difference matters
/// enormously
...
/// breaker, which is what keeps a cancelled HTTP request from being read as a database
/// failure.</para>
/// </summary>
    ...
    : DbCommandInterceptor
{
    ...
    private void Report(Exception? exception)
    {
        switch (DbErrorClassifier.Classify(exception))
        {
            ...
            // CapacityBackpressure: the server is fine and busy. Arming would issue another query
            // that
            // either adds load or cannot get a connection at all.
            // None: unique violations and ordinary bugs. This codebase *expects* PK violations -
            // the
            // retry/idempotency pairing in WorkflowDbWriteMetrics depends on them - so an
            // unqualified
