...
public sealed class NodePilotApiClient
{
    ...
    public async Task<JsonElement> ImportScorchAsync(ReadOnlyMemory<byte> xml, Guid? folderId, CancellationToken ct)
    {
        EnsureReady();
    ...
    public async Task<JsonElement> GetOperationsGraphAsync(CancellationToken ct, int windowMinutes = 30)
    {
        EnsureReady();
