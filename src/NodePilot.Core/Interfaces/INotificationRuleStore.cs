...
public interface INotificationRuleStore
{
    ...
    /// system (built-in infrastructure) management surfaces each filter on this, so neither can
    /// list or mutate the other's rows.</summary>
    ...
    /// <summary>A rule by id, but only if it is of the given kind, else null. Kind-scoped fetch for
    /// the custom and system endpoints.</summary>
    ...
    /// <summary>
    /// Replaces the rule's scalar fields and its full route and target sets with those of
    ...
    /// <summary>Decrypts a route's stored secret for the sender, or null if none is set.</summary>
