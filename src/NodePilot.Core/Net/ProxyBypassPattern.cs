...
public static class ProxyBypassPattern
{
    ...
    /// <see cref="System.Net.WebProxy"/> matches its bypass entries against the full request URI
    /// (scheme + host + port + path), not just the host — so the emitted expression anchors on
    /// the scheme and wraps the host pattern in optional port/path suffixes. Without that
    /// anchoring a bare <c>localhost</c> entry would never match anything.
    /// </summary>
