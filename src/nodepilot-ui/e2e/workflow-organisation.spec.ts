 ...
 * EN locale under Playwright; selectors are bilingual or data-testid based.
 *
 * SharedFolderTree renders the left sidebar of /workflows from GET /api/shared-workflow-folders.
 * It supports selecting a folder (filters the list by exact folderId), creating a subfolder,
 * rename, recursive delete of a folder with its contents, and multi-select with a bulk bar
 * (opt-in via `bulkDeleteEnabled`, so the designer's embedding of the same tree stays a plain
 * browser).
 *
 * The non-recursive delete answers 409 on a non-empty folder. No UI surface calls it, so that
 * contract is asserted in the dotnet controller tests and in the CLI tests.
 *
 * Covered:
 *   - 18.1 — folder hierarchy: tree renders parent and child, create subfolder, delete,
 ...
 *   - 52.4 — workflow-to-folder move (POST /api/workflows/{id}/move-folder), driven via the
 *            tree's onWorkflowDropped callback.
 *
 * Not coverable in this harness (documented skips below): dragging a workflow row onto a folder
 * (18.1) and folder-to-folder move (52.1/52.2). Both need HTML5 drag-and-drop data that
 * Playwright's synthetic dragTo does not provide, and folder move has no UI trigger at all.
 * The dotnet API tests assert them instead.
 */
