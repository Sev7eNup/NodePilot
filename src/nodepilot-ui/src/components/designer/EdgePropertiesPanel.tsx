...
import { Section } from './properties/panelChrome';
    ...
    <div className="np-anim-panel bg-surface-low flex flex-col h-full overflow-hidden shrink-0 z-10 shadow-[-20px_0_40px_rgba(0,0,0,0.02)]" style={{ width: width ?? 320 }}>
      ...
      */}
      <div className="flex-1 overflow-y-auto px-6 pb-6 pt-5 @container" style={{ containerType: 'inline-size' }}>
        ...
        <Section title={t('edgePanel.label')}>
          <div>
            <input
              type="text"
              value={label}
            ...
            />
            <p className="text-xs text-outline mt-2 leading-snug">{t('edgePanel.displayedOnArrow')}</p>
          </div>
          {labelOverridesAuto && (
            <div className="flex items-start gap-2 p-2.5 bg-warning-container/60 border border-warning/30 rounded-md text-xs">
              <div className="flex-1 min-w-0">
                <div className="text-on-warning-container font-medium">
                  {t('edgePanel.labelOverrides')}
                </div>
                <div className="text-on-warning-container/80 mt-1 truncate" title={autoLabel}>
        ...
        <Section
          title={t('edgePanel.conditionOptional')}
          action={expertMode ? (
              ...
              {sourceNode && (
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className="px-2.5 py-1 text-xs bg-success-container/70 text-on-success-container border border-success/30 rounded-md hover:brightness-95"
                  >{t('edgePanel.onSuccess')}</button>
                  <button
                    onClick={() => setCondition(`${edge.source}.failed`)}
                    className="px-2.5 py-1 text-xs bg-error-container/70 text-on-error-container border border-error/30 rounded-md hover:brightness-95"
                  >{t('edgePanel.onFailure')}</button>
                  <button
                    onClick={() => setCondition('')}
                    className="px-2.5 py-1 text-xs bg-surface-low text-on-surface-variant border border-outline-variant/50 rounded hover:bg-surface-container"
        ...
        <Section title={t('edgePanel.status')}>
          <button
        ...
        <Section title={t('edgePanel.deleteSection')}>
          <button
            onClick={async () => {
