...
/**
 * Performance tuning tab. The mode card on top decides whether the cards below apply: with
 * automatic sizing (the default), NodePilot derives the numbers from CPU and memory and the
 * configured values sit inert as a preset. Everything here is strict-startup, so a save shows
 * the restart banner, except Threading, which re-applies live while manual tuning is on. The
 * mode form lives here, not in the mode card, so the cards below react to the checkbox at once.
 */
