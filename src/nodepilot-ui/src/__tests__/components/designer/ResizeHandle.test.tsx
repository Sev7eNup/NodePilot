 ...
 * execution step list) and its neighbour. These tests pin the properties that keep it
 * findable: a grip painted without a hover prefix and a pointer target wider than the 4 px
 * lane, plus the event-forwarding contract.
 */
describe('ResizeHandle', () => {
  ...
  it('paintsTheGripAtRestNotOnlyOnHover', () => {
    ...
    // The base colour must be unprefixed. A `group-hover:`-only fill leaves the handle
    // invisible until the cursor finds it.
  ...
  it('widensThePointerTargetBeyondTheDrawnLane', () => {
    ...
    // Layout width stays 4 px so the canvas does not shift.
    expect(handle.className).toContain('w-1');
    // A transparent extender overhangs the lane on both sides.
  ...
  it('forwardsMouseDownFromTheOverhangingHitArea', () => {
    ...
    const handle = screen.getByRole('separator');
    const extender = [...handle.querySelectorAll('span')].find((s) => s.className.includes('-inset-x-1'));
    // The extender carries no handlers of its own and relies on bubbling to the wrapper.
