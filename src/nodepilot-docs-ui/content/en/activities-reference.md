...
- **Engine (local only; a remote step always runs in Windows PowerShell 5.1 on the target):**
  ...
  - `runspace` runs in the in-process PowerShell 7 pool. It is the fastest option, with no process start, but only loads modules marked Core-compatible, cannot start background jobs (`Start-Job`, `Get-WindowsUpdateLog`) and cannot be combined with `isolated`.
