...
- **Outputs:** `output`, `error`, `param.exitCode` (always), `param.*` (auto-captured declared variables)
...
- **Notes:** declared variables become `param.*` (`$hostName = …` → `{{step.param.hostName}}`). Auto-quoting: `{{step.output}}` is substituted as a single-quoted string — write `$x = {{step.output}}`, not `$x = '{{step.output}}'`.
