...
- **Config:** `script`, `engine` (`auto`/`powershell`/`pwsh`/`runspace`), `timeoutSeconds`, `isolated` (bool, local only — its own process inside a Windows job object: crash/leak containment, no orphaned processes) + optional caps `memoryLimitMb` (makes allocations fail, does not terminate) / `maxProcesses`, `successExitCodes` (comma-separated, an opt-in exit-code gate)
- **Engine (local only; a remote step always runs in Windows PowerShell 5.1 on the target):**
  - `auto` (default) and `powershell` run the script in Windows PowerShell 5.1 as a separate process. A local step therefore sees the same modules as the same script run remotely: every Windows module and every module installed for all users on the NodePilot server.
  - `pwsh` uses a PowerShell 7 process and requires PowerShell 7 to be installed.
  - `runspace` runs in the in-process PowerShell 7 pool. It is the fastest option, with no process start, but only loads modules marked Core-compatible, cannot start background jobs (`Start-Job`, `Get-WindowsUpdateLog`) and cannot be combined with `isolated`. If a script there starts Windows PowerShell itself, only the direct call `& powershell.exe` gets the right modules; one started through `cmd /c`, `Start-Process` or `[Diagnostics.Process]` lacks commands such as `New-Guid`.
- **Modules from the PowerShell Gallery** (for example `PSWindowsUpdate` with `Get-WindowsUpdate`) are not Windows modules. They have to be installed where the step runs: locally for all users on the NodePilot server, remotely on the target machine.
- **Background programs and isolation robustness:** a step that runs as a process ends when its script ends. After the script exits, NodePilot collects remaining output for at most `Engine:IsolatedDrainGraceSeconds` (default 5 s). A program the script started in the background without a new window (`Start-Process -NoNewWindow`, `cmd /c start`) keeps running but no longer holds the step. NodePilot also serializes inheritable-handle spawns, so a pipe handle leaked into a foreign process cannot block an isolated run indefinitely.
- **Outputs:** `output`, `error`, `param.exitCode` (always — the last native command **of this script**), `param.*` (the variables this script assigns). `output` holds each pipeline object as text, as on a remote step; `Write-Host`, warnings and progress are not part of it.
- **Success (error-based, locally as remotely):** a step fails as soon as the script writes a PowerShell error, including a non-terminating one (`Write-Error`, a failed cmdlet under `Continue`), or when it does not parse. In Windows PowerShell 5.1, stderr from a native program counts as an error and ends the script; to avoid that, merge the streams inside the program call: `cmd /c "tool.exe 2>&1"`. An `exit N` does not automatically mark the step as failed. For exit-code-based evaluation, set `successExitCodes`, for example `"0"`. `successExitCodes`/`param.exitCode` apply to native command codes (`$LASTEXITCODE`) in every engine; a script's own `exit N` is only visible as a value in process/isolated mode (a runspace cannot observe `exit`).
- **Notes:** variables the script assigns become `param.*` (`$hostName = …` → `{{step.param.hostName}}`), even when the script then ends with `exit` or `return` or throws. An upstream parameter the script only reads stays the output of the step that produced it, and PowerShell automatic/preference variables are never published. Auto-quoting: `{{step.output}}` is substituted as a single-quoted string — write `$x = {{step.output}}`, not `$x = '{{step.output}}'`. Inside a word or path the value becomes part of that one word, wherever the template stands: `C:\logs\{{step.output}}.txt`, `{{step.param.dir}}\app.txt` and `{{step.param.name}}.log` each stay a single argument. A method call such as `{{step.output}}.Trim()` stays a method call.
...
`stdout` and `stderr` are decoded as UTF-8 when the program wrote valid UTF-8 (or UTF-16 with a byte order mark), otherwise in the machine's OEM code page, the default of console programs. Each stream is capped at 1 MiB. When the program exceeds `timeoutSeconds`, it is ended and the step fails with the output collected until then.
...
- **Engine (script mode, local only):** `engine` with the same values as on `runScript`; default `auto` = Windows PowerShell 5.1 like a remote step. The typed modes always run in the in-process pool.
