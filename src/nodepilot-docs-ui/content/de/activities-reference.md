...
- **Engine (nur lokal; ein Remote-Step läuft immer in Windows PowerShell 5.1 auf dem Ziel):**
  ...
  - `runspace` läuft im In-Process-Pool mit PowerShell 7. Das ist am schnellsten, weil kein Prozessstart anfällt. Dafür lädt es nur Module mit Core-Kennzeichnung, kann keine Hintergrund-Jobs starten (`Start-Job`, `Get-WindowsUpdateLog`) und lässt sich nicht mit `isolated` kombinieren.
...
- **Outputs:** `output`, `error`, `param.exitCode` (immer — das letzte native Kommando **dieses** Skripts), `param.*` (die Variablen, die dieses Skript zuweist)
...
- **Notes:** Variablen, die das Skript zuweist, werden zu `param.*` (`$hostName = …` → `{{step.param.hostName}}`). Ein Upstream-Parameter, den das Skript nur liest, bleibt Output des erzeugenden Steps; PowerShell-Automatiken und Preference-Variablen werden nie publiziert. Auto-Quoting: `{{step.output}}` wird als Single-Quoted String eingesetzt — `$x = {{step.output}}` schreiben, nicht `$x = '{{step.output}}'`.
