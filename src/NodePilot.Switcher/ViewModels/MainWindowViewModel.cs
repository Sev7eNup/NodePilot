...
internal sealed class MainWindowViewModel : INotifyPropertyChanged
{
    ...
    private readonly ISwitcherConfigurationProbe _configuration;
    ...
    private string? _loggedConfigurationError;
    ...
    public MainWindowViewModel(
        ...
        ISwitcherConfigurationProbe configuration,
    ...
    public bool CanSwitchToNodePilot => !IsBusy && _configurationError is null
                                        && NodePilotAvailable && _state != EnvironmentState.NodePilotActive;
    public bool CanSwitchToSystemCenter => !IsBusy && _configurationError is null
                                           && SystemCenterAvailable && _state != EnvironmentState.SystemCenterActive;
    public bool FooterStatusIsBusy => IsBusy;
    public bool FooterStatusIsError => !IsBusy && (_lastSwitchFailed || _configurationError is not null);
    public bool FooterStatusIsWarning => !IsBusy && !FooterStatusIsError
    ...
    public string FooterStatusText => IsBusy
        ...
        : _configurationError is not null
            ? Strings.ConfigurationInvalidStatus
            : _lastSwitchFailed
    ...
    public string FooterDetailText => IsBusy && _activeSwitchTarget is { } target
        ...
        : _configurationError ?? (_lastSwitchFailed ? Strings.DetailsInActivityHistory : StateTitle);
    ...
    public async Task RefreshAsync()
    {
        ...
        try
        {
            ...
            _configurationError = await Task.Run(_configuration.Probe);
    ...
    /// <summary>
    /// Reports the configuration state once per change. A repaired file logs the system-check line
    /// again, so the activity history shows the switch is available without a restart.
    /// </summary>
    private void LogConfigurationState()
    {
        if (_configurationError is not null)
        {
            if (_loggedConfigurationError == _configurationError) return;
            _loggedConfigurationError = _configurationError;
            _initialRefreshLogged = false;
            _logger.Error(Strings.ConfigurationUnusable(_configurationError));
            return;
        }

        _loggedConfigurationError = null;
    ...
    {
        await RefreshAsync();
        // The file can break between two refreshes, so the button state alone is not a guarantee.
        if (_configurationError is not null)
        {
            _interaction.ShowError(_configurationError);
            return;
        }
