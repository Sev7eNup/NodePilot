...
/// <c>Jwt:Key</c> once at startup and injects the provider; tests mirror that contract with
/// the same 32+ character key their in-memory configs emit, so tokens minted by one component
/// validate in another. Shared here instead of duplicated across auth test files. The
/// interface is Api-side, so the type cannot move further down into TestCommons.
/// </summary>
