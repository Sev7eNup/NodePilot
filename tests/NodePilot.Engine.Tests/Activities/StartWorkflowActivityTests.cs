...
public sealed class StartWorkflowActivityWaitModeTests : IDisposable
{
    ...
    /// <summary>Captures the <c>timeoutSeconds</c> (5th positional arg) the detached child engine
    /// run is invoked with.
    /// IWorkflowEngine.ExecuteAsync has 11 params (the trailing one is <c>interactiveRun</c>), so
    /// both the
