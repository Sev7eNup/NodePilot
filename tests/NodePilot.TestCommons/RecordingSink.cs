...
/// NodePilot.Core. Dispatcher tests in Engine.Tests and alerting controller tests in Api.Tests
...
public sealed class RecordingSink(NotificationChannel channel = NotificationChannel.Email) : INotificationSink
{
    ...
    /// pre-consolidation call sites (<c>Sends[0].ctx</c>, <c>.Which.target</c>) keep
    /// compiling.</summary>
    ...
    /// <summary>Hook invoked before recording a send to inject mid-delivery races.</summary>
