...
///
/// <para>Every hosted service now takes <see cref="IDatabaseAvailability"/> so it can park instead of
/// hammering a database that is gone. Tests that exercise the service's own logic want that gate open
...
{
    /// <summary>
    /// A breaker that is always servable. Note this is a real <see cref="DatabaseAvailabilityTracker"/>
    /// rather than a stub: a stub that always returned <c>true</c> would keep passing if
    /// <c>WaitUntilServableAsync</c> were changed to throw on cancellation, which is the one behaviour
    ...
    /// <summary>A breaker whose gate is closed, for asserting that a loop parks rather than proceeds.</summary>
