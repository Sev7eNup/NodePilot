...
- `src/nodepilot-docs-ui/src/site/` — Projekt-Website (DE/EN, Vanilla-TS ohne React/Tailwind) an der Wurzel von GitHub Pages; die Doku liegt dort unter `/docs/`. Zweiter Vite-Build im Doku-Paket (`vite.site.config.ts` → `dist-site/`), **nie** Teil von `dist/` oder des Produkts. `scripts/assemble-site.mjs` setzt Website, Doku (`docs/`) und `pages-media/` (`media/`) zu `_site/` zusammen — dasselbe Skript nutzt `docs-pages.yml`. `npm run dev:site` (Port 5175) zeigt nur die Website; `npm run preview:site` ist die **einzige vollständige** lokale Vorschau. Alte Doku-Links (`#/…` an der Wurzel) leitet `public/legacy-docs-redirect.js` nach `docs/` weiter — eine neue Website-Route gehört dort **und** in `SITE_ROUTE_SEGMENTS` (`router.ts`). Impressum/Datenschutz rendern `legal/impressum.de.html` + `legal/datenschutz.de.html`; die Texte liefert der User, Claude legt sie nie an. Details: `src/nodepilot-docs-ui/README.md`
