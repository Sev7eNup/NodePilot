...
- `src/nodepilot-ui/demo/` — **Browser-Demo** der SPA für GitHub Pages (`/demo/`): dieselbe App gegen ein In-Memory-Backend, kein Server, keine DB. Eigener Entry (`demo.html` → `demo/main.demo.ts`) und dritter Vite-Build (`vite.demo.config.ts` → `dist-demo/`). Der Produkt-Build kann den Demo-Code **nicht** enthalten, weil `index.html` ihn nie referenziert — Erreichbarkeit, nicht Tree-Shaking; `ci.yml` prüft das am gebauten `dist/` per Sentinel. Abhängigkeitsrichtung ausschließlich `demo/` → `src/`: Hub-Verbindung, Doku-Link und Auth-Transport werden vom Demo-Entry **injiziert** (`src/lib/hubConnection.ts`, `src/lib/docsLink.ts`, `isolateAuthBoundaryTransport`). Seed sind die Workflow-JSONs aus `samples/` und `scripts/`; Historie und Dashboard werden daraus **abgeleitet**, nie danebengeschrieben. Details: `src/nodepilot-ui/CLAUDE.md`
...
- `src/nodepilot-docs-ui/src/site/` — Projekt-Website (DE/EN, Vanilla-TS ohne React/Tailwind) an der Wurzel von GitHub Pages; die Doku liegt dort unter `/docs/`. Zweiter Vite-Build im Doku-Paket (`vite.site.config.ts` → `dist-site/`), **nie** Teil von `dist/` oder des Produkts. `scripts/assemble-site.mjs` setzt Website, Doku (`docs/`), Browser-Demo (`demo/`) und `pages-media/` (`media/`) zu `_site/` zusammen — dasselbe Skript nutzt `docs-pages.yml`. Jede Eingabe ist Pflicht: eine optionale ließe einen Deploy die alte Demo stillschweigend weiterveröffentlichen. `npm run dev:site` (Port 5175) zeigt nur die Website; `npm run preview:site` ist die **einzige vollständige** lokale Vorschau. Alte Doku-Links (`#/…` an der Wurzel) leitet `public/legacy-docs-redirect.js` nach `docs/` weiter — eine neue Website-Route gehört dort **und** in `SITE_ROUTE_SEGMENTS` (`router.ts`). Impressum/Datenschutz rendern `legal/impressum.de.html` + `legal/datenschutz.de.html`; die Texte liefert der User, Claude legt sie nie an. Details: `src/nodepilot-docs-ui/README.md`
...
- **Doc-Sync:** Feature-Änderungen halten alle Doku-Flächen synchron — README, `docs/*.md`, `docs/testing/E2ETests.md` + `e2e/README.md` und die Doku-Website `src/nodepilot-docs-ui/content/` (eigener kuratierter Korpus, kein Render von `docs/`). **Die Doku-Website ist zweisprachig:** jede Seite existiert unter `content/de/<pfad>.md` **und** `content/en/<pfad>.md` — beide Bäume sind deckungsgleich, eine neue Seite braucht beide Dateien plus je einen Titel-Eintrag in `src/i18n/locales/{de,en}.json`. Markdown-Querverweise werden **ohne** Sprach-Präfix geschrieben (`../enterprise/folder-rbac`); die aktive Sprache setzt `DocPage` zur Laufzeit davor. Der AI-Wissenskorpus (`NodePilot.Api.csproj`) zieht bewusst **nur** `content/en/`. **Die Doku-Website hat zwei Auslieferungen:** GitHub Pages (`docs-pages.yml`) veröffentlicht sie unter `/docs/` (an der Pages-Wurzel liegt die Projekt-Website), und `wwwroot/docs` im Server-Artefakt bzw. Desktop-Paket, das die API unter `/docs` anonym bedient (`Hosting/DocsSiteSetup.cs`) — das Produkt liefert nur die Doku aus, nie die Projekt-Website. Deshalb darf `index.html` der Doku-Website **kein Inline-`<script>`** enthalten (CSP `script-src 'self'`, Guard: `document-head.test.ts`).
...
**Ein veröffentlichter Wert hat genau einen Besitzer (SCOrch-Modell).** Die qualifizierte Form `{{aktivität.param.name}}` ist die verbindliche und löst bei **jedem** Nachfahren auf, nicht nur beim direkten Nachfolger. Daneben gibt es den unqualifizierten Kurznamen (im `runScript` als `$name` injiziert) — den legt der Resolver nur an, wenn **genau eine** Aktivität auf dem Vorgängerpfad diesen Namen veröffentlicht. Bei zwei Publishern wird **nichts** gebunden, statt einen Gewinner zu ziehen: der kam früher aus der Hash-Reihenfolge der Ahnenmenge und konnte sich nach einem Prozess-Neustart ändern. Der Canvas-Linter meldet das als `dup-published-param`; im Skript bleibt der Wert über `$Params['stepA.param.name']` oder ein `{{stepA.param.name}}`-Template erreichbar. **Gemeldet wird nur, wenn mindestens ein Publisher den Namen selbst vergeben hat** (Skript-Variable, `returnData`-Key, `manualTrigger`/Webhook-Parameter, `wmiQuery`-Property). Typ-abgeleitete Namen kollidieren bauartbedingt und sind nicht umbenennbar — statische Katalog-Outputs (`exitCode` an jedem `runScript`) ebenso wie die operations-abhängigen `registryOperation`-Outputs (`value`, `type`, `values`, `count`, `subKeys`, `exists`, `created`) und das automatische `count` von `wmiQuery`. Zwei davon untereinander sind still; ein Skript-`$value` neben einem Registry-Read bleibt gemeldet, weil dort die Skriptvariable umbenannt werden kann. Herkunft der Namen: `WorkflowDataBusAnalyzer.AuthoredParameters` vs. `TypeDerivedParameters` (Frontend-Spiegel `authoredParamNames`/`typeDerivedParamNames`).

**Contract-Garantie:** Drei Muster im `VariableResolver` — `GlobalsPattern` (`globals.NAME`), `ManualPattern` (`manual.NAME`) und `StepPattern` mit genau vier Tails (`output`, `error`, `success`, `param.X`). Andere Tails bleiben als Literal. Unresolved → granulare Diagnostik (StepRunner T-7.1); ein unbekannter Trigger-Input bekommt einen eigenen Befund („Unknown trigger input(s)"), ein unbekanntes Global ebenso („Unknown global variable(s)") statt als fehlender Step gemeldet zu werden — das Globals-Muster gilt auch für `runScript`/Custom Activities, weil ein nicht existierendes Global nie legitimer Skripttext ist. Scheitert das **Laden** der Globals (Store nicht lesbar), wird ein Lauf, der Globals referenziert, vor dem ersten Step als `Failed` beendet statt mit leerem Dictionary zu laufen. Ein neuer Namespace braucht ein eigenes Muster: `manual.NAME` hat als Tail einen frei gewählten Namen und kann von `StepPattern` prinzipiell nicht getroffen werden — genau daran scheiterte es bis 1.2.7 still (Platzhalter blieb stehen, Step meldete Erfolg).
...
**Conditions sind fail-closed (ADR 0015).** Save/Publish/Import lehnen eine unbrauchbare Condition mit `400` ab (`invalid-edge-condition`, `EdgeConditionValidator` in Core): unbekannter Typ oder Operator, fehlender Operand/Child, Legacy-String außer `<stepId|alias>.success|failed`, Verweis auf einen Step, der weder Node-Id noch `outputVariable` ist. Zur Laufzeit wirft `ConditionEvaluator` bei einer solchen Condition `ConditionEvaluationException`, der Scheduler bricht den Lauf mit Kantenbezug ab (`Edge 'e1' (a -> b) has an invalid condition: …`) — die Kante wird weder geöffnet noch still übersprungen. Ein Variablen-Operand **ohne Wert** (Step ohne Ergebnis, fehlender `param`, fehlendes Global oder Trigger-Input) macht seinen Vergleich **unentscheidbar**: unentscheidbar erfüllt die Condition nie, auch nicht durch `not`, und in einer Gruppe entscheidet nur ein echtes `true`/`false` (AND: ein `false` genügt; OR: ein `true` genügt). Vorher ergaben unbekannter Typ, kaputte Legacy-Strings und `!=`/`isEmpty`/`contains ""` auf fehlenden Werten `true` und öffneten den Pfad, den die Condition schützen sollte. Alerting-Filter (`source: event`) sind davon ausgenommen: ein fehlendes Event-Feld liest als leer, weil der Feldkatalog deklariert ist.
...
- **Kein Step überlebt seine Execution:** Jede terminale Execution-Schreibung (Engine-CAS, `/cancel`, `/cancel-all`-Zombie-Pfad, Offboarding) setzt anschließend alle noch `Running`/`Paused`-Steps dieser Execution auf `Cancelled` (`ExecutionStateLifecycle.CancelOrphanedStepsAsync`). Sonst bliebe ein Step, dessen terminaler Write scheiterte, für immer `Running` — sichtbar als endloser Spinner im Execution-Panel und als dauerhaft zu hoher „aktive Läufe"-Badge auf der Maschinenseite, weil beide nur `StepExecution.Status` lesen.
...
| Browser-Demo (`demo/`), Hub-/Doku-/Auth-Naht, root-absolute URL-Literale in `src/` | `src/__tests__/demo/*` (Seed-Integrität, Backend-Vertrag, Fake-Hub, Isolation) + `e2e-demo/demo-smoke.spec.ts` | nodepilot-ui |
...
- **Zwei Wire-Dialekte, kein Config-Key:** `LlmEndpointGuard.ResolveEndpoint` leitet aus dem `BaseUrl`-Pfad ab, wohin gepostet wird und wer antwortet — `…/responses` → `OpenAiResponsesLlmClient` (OpenAI Responses API), sonst `OpenAiCompatibleLlmClient`; endet der Pfad schon auf `/chat/completions`, wird **nichts** mehr angehängt. Gemeinsames HTTP-Plumbing in `LlmHttpTransport`. Die vier Quirk-Fallbacks (`max_tokens`→`max_completion_tokens`, `stream_options`, `response_format`, `strict`) sind Chat-Completions-only und im Responses-Client bewusst nicht vorhanden; dieser sendet immer `store: false`. **Ausnahme ist `temperature`** (`LlmTemperatureQuirk`, in **beiden** Dialekten): Reasoning-Modelle lehnen Sampling-Parameter mit `400` ab, der Parameter ist optional, also wird er fallengelassen und der Aufruf einmal wiederholt — gemerkt pro `postUrl|model`, damit Folgeaufrufe den sicher scheiternden Roundtrip überspringen. Erkannt wird nur ein 400er, der `temperature` **und** eine „unsupported"-Formulierung nennt; ein 400er, der den Parameter bloß erwähnt (`temperature must be between 0 and 2`), erreicht den Aufrufer.
