...
- The project website and the docs site redeploy themselves on push to `main`
  (`.github/workflows/docs-pages.yml`); they do not need anything here. The docs' **second** copy
  does ride along in the artifacts, though: after installing, check `GET /docs` answers 301 to
  `/docs/` and that `/docs/` renders the docs without signing in. A missing bundle fails the
  install, but a broken one does not.
