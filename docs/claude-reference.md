...
| `startProgram` | Remote | `filePath` (gespeichert: absoluter lokaler Pfad; kein UNC/PATH-Lookup), `arguments`, `workingDirectory`, `useShellExecute` (durch `StartProgram:DisallowShellExecute` gesperrt, wenn aktiviert), `waitForExit`, `timeoutSeconds`, `successExitCodes`. Der Designer ergänzt `cmd`, `powershell`, `cscript`, `wscript` (jeweils auch mit `.exe`) beim Verlassen des Felds zum absoluten Pfad; API/MCP und generierte Definitionen müssen ihn selbst liefern. | `param.exitCode`, `param.processId`, `param.stdout`, `param.stderr`, `param.waited` |
