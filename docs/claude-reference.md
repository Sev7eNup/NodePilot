...
| `RestApi:AllowedHosts` | `[]` | Exakte Host-/IP-Allow-Liste für tatsächlich proxied `restApi`-Ziele/Redirects — die Ausnahme von `BlockPrivateNetworks`. Link-Local/Cloud-Metadata bleibt immer gesperrt (Dev: `localhost`/`127.0.0.1`/`::1`). **Die Sperre gilt der Adresse, nicht dem Namen:** löst ein Host auf mehrere Adressen auf, genügt **eine** brauchbare (`NetworkGuard.AssertAnyAddressUsable`) — ein domänengebundener Windows-Host registriert seine IPv6-Link-Local-Adressen neben den routbaren im DNS und wäre sonst dauerhaft unerreichbar, ohne Möglichkeit ihn freizuschalten. Verbunden wird trotzdem nie zu einer gesperrten Adresse: `EnforceConnect` wendet dieselbe Regel im `ConnectCallback` pro Adresse an und öffnet den Socket nur zu einer, die dort überlebt |
