...
| Browser shows **Bad Request - Invalid Hostname** / `HTTP Error 400. The request hostname is invalid.` | the page looks like IIS but comes from ASP.NET Core's host filtering: the name in the address bar is not in `AllowedHosts`, which the installer sets to the **public hostname** it was given plus `localhost` and nothing else. A short name, an IP or a DNS alias all land here | browse the exact name — the FQDN also matches the certificate, so it settles the trust prompt too. `install-report.txt` names the list. To add one permanently, extend `AllowedHosts` (semicolon-separated) in `appsettings.Production.json` and restart the service; the list is read once at boot. `"*"` is refused — `Security:StrictAllowedHosts` is `true` in the production template and aborts the boot — but `*.example.com` is accepted |
...
| `np auth login` fails with *Netzwerk-Fehler: TLS-Verbindung … nicht möglich* (older builds: only `The SSL connection could not be established, see inner exception.`) | same cause, but a browser can be clicked through and a CLI cannot: the certificate does not chain to a root this machine trusts | the message names the presented certificate and its SHA-256. Either import the certificate into `LocalMachine\Root`, or pin that one certificate: `np auth login --server <URL> --tls-thumbprint <SHA-256>` (stored in the profile; the MCP server reads `NODEPILOT_MCP_TLS_THUMBPRINT`). `--insecure-tls` skips the check for a single call and is never stored |
| `np` fails with *Der Hostname steht nicht in den Zertifikatsnamen* | the URL names a host the certificate does not carry — running `np` on the server itself against `https://localhost` is the common one, because the certificate names the public host name | point the CLI at a name the certificate carries: `np config set server https://<a name from the certificate>:<port>`. The message lists those names and prints the command. A `LocalMachine\Root` import does **not** fix a name mismatch; pin it only when the host has to stay as it is (reverse proxy, alias) |
