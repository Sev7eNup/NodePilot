...
The NodePilot Switcher is a small, local Windows utility for machines that host both
...
2. `%ProgramData%\NodePilot\Switcher\switcher.json`
3. `switcher.json` next to the executable
...
template to `%ProgramData%\NodePilot\Switcher`.
...
next to the executable; a machine-wide configuration under `%ProgramData%\NodePilot\Switcher`
...
{
  "nodePilot": {
    "workflowAllowListPath": "C:\\ProgramData\\NodePilot\\Switcher\\nodepilot-workflows.txt",
    "cliPath": null,
    "profile": "switcher",
    "serverUrl": null,
...
& 'C:\Program Files\NodePilot\tools\np\np.exe' auth login `
  --profile switcher --server https://nodepilot.example.test
```
...
log is stored under `%ProgramData%\NodePilot\Switcher` and is restricted to Administrators
and SYSTEM.
...
The server installer adds **NodePilot Switcher** to the Start menu. In a scripted ZIP
installation, run:

```text
C:\Program Files\NodePilot\tools\switcher\NodePilot.Switcher.exe
```

Release builds also publish `NodePilot-Switcher-<version>-win-x64.zip`, containing the
...
The utility was called *Service Switcher*, then *Engine Switcher*, and is now the *Switcher*. Each
rename moved every persisted name. The installer places the new files, but nothing carries the old
state over — on a machine that already ran a previous version, do this once after updating:

```powershell
$new = "$env:ProgramData\NodePilot\Switcher"
foreach ($old in "$env:ProgramData\NodePilot\EngineSwitcher", "$env:ProgramData\NodePilot\ServiceSwitcher") {
    if ((Test-Path $old) -and -not (Test-Path $new)) { Rename-Item -LiteralPath $old -NewName 'Switcher' }
}
foreach ($name in 'engine-switcher.json', 'service-switcher.json') {
    $config = Join-Path $new $name
    if (Test-Path $config) { Rename-Item -LiteralPath $config -NewName 'switcher.json' }
}
```

A configuration that sits next to the executable instead needs the same rename in
`{app}\tools\switcher`. The old `{app}\tools\engine-switcher` and `{app}\tools\service-switcher`
directories and the previous Start-menu entries are left behind by the update and can be deleted.

The CLI profile in the shipped template is now `switcher`. An existing installation may keep
...
The theme preference moves to `HKCU\Software\NodePilot\Switcher`; the old keys are ignored and
