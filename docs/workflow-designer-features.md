...
- **Global assistant launcher** (the chat button in the bottom-right corner, present on every page including the designer): opens the read-only knowledge chat as a widget over the canvas. It and the workflow assistant open exclusively of one another — opening either closes the other — and the two keep separate conversations. Visibility follows `GET /api/ai/knowledge/capabilities`; described fully in `docs/ai-features.md`.
