...
| `llmQuery` | **LLM-Abfrage** — OpenAI-kompatibler Endpunkt (Prompt→Text; Chat Completions oder Responses-API, je nach `baseUrl`-Pfad). **prompt** (Pflicht, `{{templates}}`), **systemPrompt**, **jsonMode**; Override-Abschnitt: **baseUrl**, **model**, **apiKey** (Secret), **maxTokens**, **temperature** (0–2), Timeout-Feld auto. Leer → das aktive LLM-Profil; braucht `Llm:Enabled=true` + ein auflösbares aktives Profil |
...
- **Farb-Skin-Umschalter** (Palette-Icon, icon-only, neben dem KI-Button): öffnet ein Popover mit allen 7 Skins plus `system` aus der `THEMES`-Registry. Auswahl synchronisiert sich mit dem Einstellungs-Picker und der Sidebar (gemeinsamer `useThemeStore`); Aktiv-Skin erhält ein Check-Icon; Außen-Klick schließt das Popover.
