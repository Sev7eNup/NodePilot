...
| # | Posten | Inhalt |
|---|---|---|
| 16 | **Alert → Workflow (Auto-Remediation)** | Dritter Route-Target-Typ „Workflow" im bestehenden `NotificationDispatcher` — **kein** neuer `ITriggerSource`. Exactly-once und Leader-Gating löst das Delivery-Ledger bereits. Macht aus Alerting Auto-Remediation. Bestes Aufwand/Wirkung-Verhältnis im gesamten Ideenbestand. |
...
| **Retry-Verstärkung bei DB-Timeouts eingrenzen** | Ein zweiter Endpunkt fällt durch dieselbe Falle auf. `EnableRetryOnFailure(maxRetryCount: 5)` in `DbContextSetup` stuft einen Command-Timeout als transient ein und wiederholt ihn — bei den 120 s aus `Database:CommandTimeoutSeconds` werden aus einem langsamen Query bis zu sechs volle Timeouts hintereinander. Für die Workflow-Liste ist das mit einem eigenen 15-s-Budget entschärft (2026-08-03), **für alle anderen Leseendpunkte nicht**. Die saubere Lösung trennt Lese- von Schreibpfaden: Schreibvorgänge profitieren real von der Wiederholung, interaktive Lesevorgänge nie. Vorher nicht angefasst, weil eine globale Senkung Schreibvorgänge schlechter macht und es bislang genau einen belegten Fall gibt. |
