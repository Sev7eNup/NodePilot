...
Am 2026-09-16 kamen die offenen Restposten aus dem Repository-Audit (August/September 2026) hinzu,
der bis dahin als `audit.md` im Repo-Root lag; jeder Posten ist vorher am Code gegengeprüft.
...
| **Workflow-Liste über 500 Einträge** — `GET /api/workflows` endet bewusst bei 500 ohne Fortsetzung (DoS-Guard, gepinnt durch `GetAll_CapsResultSetAt500_EvenWhenMoreWorkflowsExist`); UI-Suche und Ordnerfilter arbeiten lokal, MCP meldet `totalAvailable` | Eine Installation mit mehr als 500 zugänglichen Workflows, z. B. ein entsprechend großer SCOrch-Export. Dann als Feature über SPA, CLI und MCP bauen — Vorbild ist die Executions-Pagination. Bis dahin verschwinden darüber nur die am längsten unberührten Workflows aus Listen und Pickern; Deep-Links und `/export` bleiben. Kleiner Zwischenschritt ohne Auslöser: im MCP `totalAvailable` durch ein `truncated`-Flag ersetzen. |
...
| `manualChunks` in `vite.config.ts` · Locales getrennt laden (DE+EN ≈ 385 KB roh liegen beide im Eager-Pfad) | Sobald NodePilot außerhalb des LAN erreichbar gemacht wird oder Initial-Load über VPN gemeldet wird. **Der Rest dieses Postens ist erledigt**: Route-Level `React.lazy` deckt 16 von 20 Seiten ab, Material Symbols kommen im Build gar nicht mehr vor, und ECharts ist am 2026-09-11 aus dem Boot-Chunk gezogen worden (Eager-Pfad 591 → 386 KB gzip, siehe [`performance-improvements.md`](performance-improvements.md)). Beim Locale-Posten zuerst den Startup-Wasserfall bewerten — ein asynchroner i18n-Init muss vor dem ersten Render awaited werden. |
| React Compiler aktivieren (`babel-plugin-react-compiler`) | Wenn wieder breit an Designer-Renderpfaden gearbeitet wird. Heute läuft **nur** die Lint-Hälfte: `eslint-plugin-react-hooks` v7 mit `flat.recommended`, während `vite.config.ts` ein blankes `react()` fährt und das Babel-Plugin nicht installiert ist — es gibt also **keine** Auto-Memoisierung. Der Code ist dadurch weitgehend compiler-sauber (Ausnahme: `react-hooks/set-state-in-effect` ist bewusst abgeschaltet), aber jede Memo-Grenze ist Handarbeit. Braucht die volle Frontend- **und** E2E-Suite, und löst die algorithmischen Posten ohnehin nicht. |
...
| N+1 bei Notifications, Workflow-Statistiken und Ordner-Grants bündeln | Aus den Audits 2026-08-15 (P2) und 2026-09-05 (M2). Ziel-Größenordnung laut Betreiber: ~100 parallele Läufe. Ordner-Grants: `ResourceAuthorizationService.GetAncestryGrantsAsync` fragt je unterschiedlichem Ordner einmal ab — nur für Non-Admins, durch das 500er-Cap begrenzt; Kandidat wäre der Folder-Tree-Endpoint, der mit der Gesamtordnerzahl skaliert. Auslöser: gemessene Latenz, nicht Verdacht — vorher `docs/performance-improvements.md` gegenlesen. |
| Step-Writes DB-seitig fencen (HA) | Aus dem Audit 2026-08-26. Execution-Terminalwrites laufen über CAS, Step-Writes über normales `SaveChanges`; `ClusterFencingHost` cancelt kooperativ, und verwaiste `Running`-Steps räumt `ExecutionStateLifecycle.CancelOrphanedStepsAsync` weg. Auslöser: ein beobachteter Step-Write eines abgesetzten Leaders nach einem Failover. |
| Audit- und SIEM-Zustellung über eine Outbox absichern | Aus dem Audit 2026-08-26. `AuditWriter` bricht eine Mutation bei einem Schreibfehler bewusst nicht ab — eine erfolgreiche Mutation garantiert deshalb keinen Audit-Datensatz. Auslöser: eine Compliance-Anforderung, die einen lückenlosen Audit-Trail je Mutation verlangt. |
| Migrations-Identität von der Laufzeit-Identität trennen | Aus dem Audit 2026-08-26. Die Laufzeit-Identität braucht heute `db_owner` auf der NodePilot-Datenbank, weil sie beim Boot migriert. Auslöser: ein Kunden-DBA verlangt die Trennung. |
| Strikte E2E-Fixture: ungemockte `/api`-Aufrufe lassen den Test scheitern | Aus dem Audit 2026-09-05 (T1). `e2e/fixtures/mockApi.ts` beantwortet nicht gemockte Aufrufe mit `200 []` — dokumentierte Konvention (`e2e/README.md`). Die Lücke betrifft Read-only-Endpoints, deren Payload keine Spec prüft; Objekt-Endpoints scheitern mit `[]` sichtbar. Auslöser: ein Fehler, der durch die Catch-all-Antwort gerutscht ist. Vorher alle Specs auf beiläufige Aufrufe durchsehen. |
| Baum-Algorithmen der beiden Ordnerbäume einmal implementieren | Aus dem Audit 2026-09-05. `RecomputePathsRecursive`, `IsDescendant` und `DescendantMaxDepth` stehen doppelt in `SharedWorkflowFoldersController` und `GlobalVariableFolderStore`. Getrennte Services wegen unterschiedlicher Autorisierung bleiben richtig, doppelte Algorithmen nicht. Auslöser: der nächste Bug, der nur eine der beiden Kopien trifft. |
| Prozessweiten Zustand der `WorkflowEngine` in einen DI-Singleton überführen | Aus dem Audit 2026-09-05. Statische Dictionaries (laufende Executions, Cancel-Gründe, User-Zähler, Debug-Handles) zwingen die Engine-Tests in die serielle `SerialEngineTestCollection`. Auslöser: die Engine-Suite wird in CI zum Zeitfaktor, oder ein Test flakt am geteilten Zustand. |
| Kleine Restposten aus dem Audit 2026-09-05 | (a) `GlobalVariableFoldersController.MapError`: `_ => throw ex` setzt den Stacktrace neu, und `InvalidOperationException` wird pauschal als Eingabefehler (400) gewertet. (b) Switcher: `ActivityLogger` hält die Historie unbegrenzt im Speicher, `MainWindowViewModel` baut die Liste bei jedem Eintrag komplett neu — Ringpuffer und inkrementelle Aktualisierung. (c) Doku-Website: `Toc.tsx` liest die Überschriften bei einem Sprachwechsel derselben Seite nicht neu (der Effekt hängt nur an `path`), und `resolveDocHref` in `DocPage.tsx` verwirft das Fragment seitenübergreifender Links. Auslöser: mitnehmen, wenn ohnehin an der jeweiligen Datei gearbeitet wird. |
...
| `BackupRestoreService` zerlegen (~1500 LOC) | Aus dem Audit 2026-09-05. Der Restore ist korrekt und getestet. **Wiederaufnahme**, wenn ein neuer Backup-Bereich nicht mehr sauber hineinpasst — dann entlang eines Restore-Planers schneiden. |
| `Install-NodePilot.ps1` zerlegen (~1850 LOC: ACLs, Zertifikatrechte, SCM-Anlage, Rollback) | Aus dem Audit 2026-09-05. Durch `Test-SetupAdapter.ps1` und `Test-DeploymentTemplates.ps1` abgesichert. **Wiederaufnahme**, wenn eine Installer-Änderung an der Größe scheitert; die Schnittgrenze ist der vorhandene `SetupContract`. |
